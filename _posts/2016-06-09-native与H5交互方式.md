...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;自从 iOS 7 之后，就引入了 JavaScriptCore 库。它是一种JavaScript引擎，主要为webkit提供脚本处理能力（其主要以safari浏览器为代表）。JavaScriptCore是开源webkit的一部分。它把 WebKit 的 JavaScript 引擎用 OC 封装，提供简单、快速、安全的方式，让两者交互。自己编写了个 demo 来总结它们之间的交互[OC 与 js 交互 Demo](https://github.com/benlinhuo/JavaScript-WebView)。
